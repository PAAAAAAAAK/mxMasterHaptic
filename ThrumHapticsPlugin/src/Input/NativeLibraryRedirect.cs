namespace Loupedeck.ThrumHapticsPlugin.Input
{
    ...
    ///   Plugin 'ThrumHaptics' uninstallation failed - Plugin was not fully uninstalled
    ///
    ...
    internal static class NativeLibraryRedirect
    {
        ...
        public static void Configure(String pluginBinDirectory)
        {
            ...
            try
            {
                ...
                {
                    PluginLog.Error($"[ThrumHaptics] Native library not found at '{source}'.");
                    return;
                }
                ...
                PluginLog.Info($"[ThrumHaptics] Native library redirected to '{target}'.");
            }
            ...
            catch (Exception ex)
            {
                ...
                PluginLog.Error($"[ThrumHaptics] Could not redirect native library: {ex.Message}");
            }
        }
        ...
        private static String PrepareCopy(String source)
        {
            ...
            var directory = Path.Combine(Path.GetTempPath(), "ThrumHaptics", "native", version);
            ...
            {
                PluginLog.Error($"[ThrumHaptics] Could not stage native library: {ex.Message}");
                return null;
            }
        }
    }
